...
/obj/structure/roguemachine/titan/proc/give_tax_popup(mob/living/carbon/human/user)
	...
	if(newtax)
		...
		priority_announce("The new tax in the town shall be [newtax] percent.", "The Generous Duke Decrees", pick('sound/misc/royal_decree.ogg', 'sound/misc/royal_decree2.ogg'), "Captain")
...
/proc/make_outlaw(raw_message, silent = FALSE)
	if(raw_message in GLOB.outlawed_players)
		...
		if(!silent)
			priority_announce("[raw_message] is no longer an outlaw in the town.", "The Duke Decrees", 'sound/misc/royal_decree.ogg', "Captain")
		return FALSE
	var/found = FALSE
